...
public class PlaceService {
    ...
    private String order = " ORDER BY "+Place.TABLE+"."+Place.KEY_name;
    ...
    public ArrayList<Place> getPlaceListByActivity(int aId){
        ...
        String selectQuery="SELECT "+
                ...
                " AND "+Activity.TABLE+"."+Activity.KEY_ID+" = "+aId+order;
        ArrayList<Place> placeList=new ArrayList<Place>();
        Cursor cursor=db.rawQuery(selectQuery,null);
    ...
    public ArrayList<String> getRegionList(){
        SQLiteDatabase db=dbHelper.getReadableDatabase();
        String selectQuery="SELECT DISTINCT "+Place.KEY_region+" FROM "+Place.TABLE+" ORDER BY "+Place.KEY_region;
    ...
    public ArrayList<String> getFacilityList(){
        SQLiteDatabase db=dbHelper.getReadableDatabase();
        String selectQuery="SELECT DISTINCT "+Place.KEY_facility+" FROM "+Place.TABLE+" ORDER BY "+Place.KEY_facility;
        ArrayList<String> faciList=new ArrayList<String>();

        Cursor cursor=db.rawQuery(selectQuery,null);

        if(cursor.moveToFirst()){
            do{
                String facility = cursor.getString(cursor.getColumnIndex(Place.KEY_facility));
                ArrayList<String> facies = Utils.getSingleElement(facility);
                for(String f: facies){
                    f = f.trim();
                    if(!faciList.contains(f)){
                        faciList.add(f);
                    }
                }
            }while(cursor.moveToNext());
    ...
    public ArrayList<Place> searchPlace(int aID, Filter filter){
        ...
        if(keyword!=null && !keyword.isEmpty()){
            selectQuery = selectQuery + "AND ("+ Activity.TABLE+"."+Activity.KEY_name+" LIKE '%"+keyword+"%' OR "+
                    ...
                    Place.TABLE+"."+Place.KEY_region+" LIKE '%"+keyword+"%' OR "+
                    Place.TABLE+"."+Place.KEY_suburb+" LIKE '%"+keyword+"%' OR "+
        ...
        ArrayList<Place> placeList=new ArrayList<Place>();
        Cursor cursor=db.rawQuery(selectQuery+order,null);
